...
module.exports = {
  ...
  usage: 'user [mention/username/userID]',
  category: 'utility',
  ...
  async execute(message, args, client) {
    ...
    if (targetInput) {
      ...
      if (/^\d{17,19}$/.test(targetInput)) {
        try {
          ...
          return await message.reply('❌ User not found with that ID.');
        }
      }
      ...
      // Try to find by username
        ...
        if (foundMember) {
          ...
          return await message.reply('❌ User not found with that username.');
        }
      }
    } else {
    ...
    if (member) {
      ...
      embed.addFields(
        ...
        { name: '🎭 Roles', value: member.roles.cache.size > 1 ? member.roles.cache.filter(role => role.id !== message.guild.id).map(role => role.toString()).join(', ').slice(0, 1024) : 'No roles', inline: false }
      );
    } else {
      embed.addFields(
